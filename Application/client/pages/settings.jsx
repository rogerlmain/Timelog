...
import { isset, is_null, not_empty, not_set } from "client/classes/common";
...
export default class SettingsPage extends BaseControl {
	...
	constructor (props) {
		...
		OffshoreModel.get_tokens ().then (response => this.setState ({ repositories: response ?? null }));
	...
	account_options_panel = () => {
		...
		let has_repos = not_empty (this.state.repositories);
		...
		return <Container id="account_options_container" visible={this.state.current_panel == options_panels.account_options}>
			...
			<div className="two-column-newspaper with-headspace">
				...
				<div>
					...
					<ContentsPanel index={has_repos ? 1 : 2}>
						<div>{has_repos && this.repository_list ()}</div>
